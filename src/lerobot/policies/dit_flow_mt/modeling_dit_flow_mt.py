...
class DiTFlowMTPolicy(PreTrainedPolicy):
    """
    Multi-task DiT Flow Policy using CLIP language conditioning and DINOv2 vision encoder.
    """
...
class DiTFlowModel(nn.Module):
    ...
    def _prepare_global_conditioning(self, batch: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if USE_STATE_PROJ:
            states = einops.rearrange(batch[OBS_STATE], "b s ... -> (b s) ...", b=batch_size, s=n_obs_steps)
